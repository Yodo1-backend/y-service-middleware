...
middleware.createClient = function ({port, host, password, db, time, errListener, success}) {
    if (!port || !host) {
    ...
    client.auth(password, function (err) {
        if (!err) {
            ...
            if (success && typeof success === 'function') {
                success('Plugin Redis init successful with config ', {
                    port: port,
                    host: host,
                    pass: password,
            ...
            throw new Error('Redis connect failed, err: ', err);
        }
    });
