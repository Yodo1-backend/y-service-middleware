...
middleware.createClientWithConfig = function (_config, options, success) {
        ...
        if (Object.keys(typeKeys).indexOf(type) >= 0) {
            try {
                if (!poseidonConfigClients[type]) {
                    ...
                    var clientInstance = poseidonClass.CreateAsTcpClient(serviceUrl, options);
                ...
                throw new Error('Poseidon catch: ' + JSON.stringify({
                    type: type,
                    url: url,
                    params: params,
                    err: err
...
middleware.createClient = function () {
    var client = {};
    client.postPoseidon = async function (domain, url, options, params) {
        if (!domain) {
            throw new Error('Domain can\'t be null');
        }
        try {
            if (!poseidonClients[domain]) {
                poseidonClients[domain] = poseidonClass.CreateAsTcpClient(domain, options);
            }
            ...
            throw new Error('Poseidon catch: ', {
                err: err,
                domain: domain,
                url: url,
                params: params
